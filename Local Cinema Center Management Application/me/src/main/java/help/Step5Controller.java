...
public class Step5Controller {
    ...
    private void handleSaveTicketInvoice(ActionEvent event) 
    {
        try 
        {
            ...
            String documentsPath = "C:\\Users\\emiro\\Documents\\GitHub\\-Local-Cinema-Center-Management-Application\\Others\\Invoices-Tickets";
    ...
    private void createInvoicePDF(String filepath, Tickets ticket, ShoppingCart cart) throws IOException {
        try (PDDocument document = new PDDocument()) {
            ...
            try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
                ...
                contentStream.beginText();
                contentStream.newLineAtOffset(margin, yPosition);
                contentStream.showText(String.format("Seat Price Total: $%.2f", ticket.getTotalSeatCost()));
                contentStream.endText();
